...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # 5/17/2017 Bhinesh Patel

        a_nodes = []

        
        for action in self.all_actions:
            # Create an action Node for this action
            node_a = PgNode_a(action)

            # Check if preconditions for this action are present in the previous literal state level.
            if node_a.prenodes.issubset(self.s_levels[level]):

                # Action is reachable so add it to the list of action nodes for this level.
                a_nodes.append(node_a)

                # action node is a child of the preceding level's state nodes
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    # parents of action node are the previous level's state nodes
                    node_a.parents.add(node_s)

        # Add all reachable action nodes to action level.
    ...
    def add_literal_level(self, level):
        ...
        # Get all the actions in the previous level
        for node_a in self.a_levels[level - 1]:
            ...
            for node_s in node_a.effnodes:
                ...
                # preceding level's action node is a parent of this reachable effect node
                node_s.parents.add(node_a)
                
                # effect node is a child of the preceding action node                               
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if node_a1's add effects negate node_a2's rem effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        # Check if node_a2's add effects negate node_a1's rem effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check if node_a1's add effects negate node_a2's neg preconditions        
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.precond_neg:
                return True

        # Check if node_a2's add effects negate node_a1's neg preconditions
        for effect in node_a2.action.effect_add:
            if effect in node_a1.action.precond_neg:
                return True

        # Check if node_a1's rem effects negate node_a2's pos preconditions    
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True

        # Check if node_a2's rem effects negate node_a1's pos preconditions 
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # 5/17/2017 Bhinesh Patel
        # Check if node_a1's preconditions are mutually exclusive with node_a2's preconditions. Only need to find one.
        for pre_node_a1 in node_a1.parents:
            for pre_node_a2 in node_a2.parents:
                if pre_node_a1.is_mutex(pre_node_a2):
                    return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # 5/17/2017 Bhinesh Patel
        # Check if two state literals are negative of each other.
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # 5/17/2017 Bhinesh Patel
        # Check if actions that lead to node_s1 can also lead to node_s2
        for actions_s1 in node_s1.parents:
            for actions_s2 in node_s2.parents:
                if not actions_s1.is_mutex(actions_s2):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        # Find first state the goal is found in and return the level as the cost.
